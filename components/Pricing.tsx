  ...
  return (
    <section id="planes" className="relative overflow-hidden px-5 py-20 md:px-8 md:py-28">
      ...
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.72fr_1.28fr] lg:items-center">
        ...
        <div className="grid gap-5 xl:grid-cols-[1fr_1fr_1fr_0.75fr]">
          {plans.map((plan) => (
            <article key={plan.name} className={`relative overflow-hidden rounded-[24px] border p-7 shadow-soft transition hover:-translate-y-1 ${plan.surface}`}>
              {plan.highlighted ? <div className="absolute inset-x-8 top-0 rounded-b-md bg-gradient-to-r from-[#ff2d87] to-[#ff8a24] px-3 py-1 text-center text-xs font-black uppercase text-white">Más elegido</div> : null}
              <span className={`mt-3 inline-block text-sm font-black uppercase ${plan.color}`}>{plan.name}</span>
              <h3 className="mt-2 text-lg font-bold text-white/80">{plan.subtitle}</h3>
              <p className="mt-3 text-sm leading-6 text-white/58">{plan.description}</p>
              <p className={`mt-5 text-3xl font-black ${plan.color}`}>{plan.price}</p>
              ...
              <a
                href="#contacto"
                className={`mt-6 inline-flex w-full items-center justify-center px-4 py-3 text-sm font-black ${
          ...
          <aside className="rounded-[24px] border border-white/12 bg-white/10 p-7 text-center shadow-soft backdrop-blur-xl xl:self-center">
            ...
            <h3 className="mt-5 text-lg font-black uppercase text-white">Financiación a medida</h3>
            <p className="mt-3 text-sm leading-6 text-white/66">Consultanos por planes de pago y cuotas.</p>
          </aside>
        </div>
      </div>
    </section>
